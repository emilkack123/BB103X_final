...
def merge_data(hydro_file, pi_file, mol_weight_file):
    """ Merge hydrophobicity, pI, and molecular weight data based on Sequence_ID and Sequence """
    ...
    # Rename Sequence_ID in molecular_weight.csv to ID for consistency
    mol_weight_df.rename(columns={"SEQUENCE_ID": "ID"}, inplace=True)

    # Merge datasets on ID and Sequence
...
def parse_args():
    parser = argparse.ArgumentParser(description='Merge hydrophobicity, pI, and molecular weight results.')
...
def main():
    ...
    # Merge the data
    merged_df = merge_data(args.hydro_file, args.pi_file, args.mol_weight_file)
